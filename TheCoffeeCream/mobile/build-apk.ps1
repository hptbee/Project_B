...
Write-Host "----------------------------------------"
Write-Host "[INFO] The Coffee Cream - Build Tool"
...
# 1. Build Web Assets
Write-Host "`n[STEP 1] Building Web Assets..."
...
# 2. Sync Capacitor
Write-Host "`n[STEP 2] Syncing Capacitor..."
Push-Location $appDir
...
Write-Host "`n[STEP 3] Building Android APK..."
...
Write-Host "`n[STEP 4] Exporting to Publish directory..."
...
if ($apkFiles) {
    ...
    Write-Host "`n[SUCCESS] BUILD SUCCESSFUL!"
    Write-Host "[OUTPUT] APK Location: $destPath"
} else {
    Write-Error "[ERROR] Could not find generated APK in $sourceDir"
    exit 1
}
